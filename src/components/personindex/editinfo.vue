    ...
    <mu-container>
        <mu-form ref="form" :model="validateForm" class="mu-demo-form" label-position="left">
          ...
          <mu-form-item label="签名" >
            <mu-text-field v-model="validateForm.motto" ></mu-text-field>
          </mu-form-item>

          <mu-form-item label="简介" >
            <mu-text-field  v-model="validateForm.introduction" ></mu-text-field>
          </mu-form-item>
          ...
          <mu-form-item label="地址">
            <mu-list>
              <mu-list-item-title @click="showAddpicker=!showAddpicker">{{validateForm.address}}</mu-list-item-title>
            </mu-list>
            <mu-dialog title="选择地址" width="360" :open.sync="showAddpicker">
          ...
          <mu-form-item label="生日">
            <mu-list>
              <mu-list-item-title @click="showTimepicker=!showTimepicker">{{validateForm.birthday}}</mu-list-item-title>
            </mu-list>
            <mu-dialog title="选择日期"  :open.sync="showTimepicker">
               <mu-date-picker  @change='birthdayChange'></mu-date-picker>
            <mu-button slot="actions" flat color="primary" @click="closeSimpleDialog">确定</mu-button>
            </mu-dialog>
          </mu-form-item>
        </mu-form>
  ...
  methods: {
    ...
    closeSimpleDialog () {
      if (this.showAddpicker) {
        this.validateForm.address = this.address.join('-')
        this.showAddpicker = false
      }
      if (this.showTimepicker) {
        this.validateForm.birthday = timeFormat(this.birthday, 'yyyy-mm-dd')
    ...
    async udpateUserdata () {
      ...
      if (!result) return
      const res = await http('/editinfo', {
        data: {...this.validateForm, userid: this.$store.state.userdata.userid},
        method: 'post'
      })
      console.log(res)
    }
